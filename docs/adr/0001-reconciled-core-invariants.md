...
Per-asker decisions/timeouts and evaluator input isolation are implemented. `integrationMode` is persisted but auto/manual behavior is not yet enforced; every merge currently follows the explicit approval path. See `BLOCKED_TASKS.md` for the remaining policy work.
