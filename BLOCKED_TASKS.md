...
- [ ] Author the deletion-semantics ADR distinguishing authorized source-custody purge from portable append-only history (DESIGN C12). Required before any ingestion deletion task starts. Drafted as ADR 0007, currently **proposed**. Accepting that ADR permits T-10-04 to build a **fail-closed framework only**; it authorises no erasure by itself. Destructive execution additionally requires (a) Max's D9 retention ratification and (b) every capability producer and adapter — graph/validity gateway, invalidation path, provider adapters, certificate signer — present and exercised by deterministic fakes. Any missing or unknown producer means no erasure.
