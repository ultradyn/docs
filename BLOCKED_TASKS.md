...
Adopted by ADR 0005/0006; execution truth lives in the backlog (phases R0–R4). Only R0/R1 (bundle M0–M3, 48 tasks: 46 bundle leaves plus synthetic splits N3 (policy contract) and T-10-04 (replay-capsule deletion, blocked on the C12 ADR)) is instantiated; the items below are the release-truth gates that later backlog epic stubs cross-link and must clear before those stubs expand.
...
- [ ] Author the deletion-semantics ADR distinguishing authorized source-custody purge from portable append-only history (DESIGN C12). Required before any ingestion deletion task starts. Drafted as ADR 0007, currently **proposed**. Accepting that ADR records **architecture only and unlocks no work**. T-10-04 includes destructive implementation and a deletion drill, so it **remains blocked as scoped** until all of: the ADR is accepted, Max ratifies D9 retention policy, and every capability producer and adapter — graph/validity gateway, invalidation path, provider adapters, certificate signer with verification/rotation/revocation, a `DeletionAuthorityVerifier`, and a revisioned `CustodyInventoryAuthority` enumerator — exists and is exercised by deterministic fakes covering authority failure (stale/expired/revoked/unverifiable/self-asserted/wrong-scope/unknown) and inventory failure (missing source, omitted location class, stale revision, scan outage, partial enumeration, unknown completeness). Until D9 explicitly delegates eligible roles, Max is the sole eligible approver for class-1 destructive execution. Any missing or unknown producer means no erasure. Framework-only work, if wanted earlier, requires a separately scoped non-destructive task; T-10-04 must not be quietly rescoped.
...
- [ ] Select and wire the production policy-attestation authority (`P2.M1.E4.T004`). T-13-01 intentionally has no permissive authority default: its deterministic issuer/verifier fake supports local R0/R1 tests only. **True production** remains fail-closed until a Max-approved ADR selects the trust root and proof model, and the task implements issuer/verifier runtime wiring, revision rotation, revocation, expiry, recovery, visible unavailable state, and the exact activation checklist. **Scope correction (Max, 2026-07-19):** this gate is true-production only. It does **not** block R0/R1 work, and it does **not** block trusted-participant alpha runs — those may run under an explicitly named, mode-gated alpha authority that still traverses attest/verify, so the control path is exercised. Alpha is trusted-operator, **not adversary-resistant**, and must never be labeled or reachable as production. The `PolicyAttestationAuthority` seam is retained regardless so a real trust root (local key / GPG / git signing) is a later addition rather than a rewrite. No alpha authority is wired today because nothing yet requires one; wire it at first genuine need rather than leaving an unused bypass in the tree.
