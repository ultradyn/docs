...
Docs 02/03 had the Matcher deduplicating new questions against _deferred_ and _answered_ only. Two people asking the same in-flight question would create duplicate records and duplicate answering work — the exact failure the Matcher exists to prevent, in the window where it's most likely (hot topics get asked repeatedly _while_ unanswered). Fix: Matcher checks active + deferred + answered; an active match attaches the new asker to the in-flight question.

**2. The Simulated Asker lost the chat log.** (Fixed — 02, 03, HTML.)
The design conversation explicitly concluded the asker-simulation must run against the raw question _plus the surrounding chat log_, because the cleaned question launders away the asker's context. The roster and flows had drifted to "raw question + goals". Fix: chat log restored to the Simulated Asker's inputs everywhere. This was a live demonstration of the context-laundering failure the check exists to catch.
...
- Fresh-context mandate: C6 ↔ roster "isolated ∎" rows ↔ 03 §7.3.2 — consistent; Structurer correctly _exempted_ (it shares the answer session by design). ✔
