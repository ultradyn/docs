...
**Who may approve.** Until Max ratifies D9 _and_ explicitly delegates eligible roles, **Max is the sole eligible approver for class-1 destructive execution** (class 3 is already Max-authorised). D9 may later name delegated roles and their scopes; delegation is always explicit, and an actor is never eligible by implication.

**Authority must be verified, not asserted.** A mandatory `DeletionAuthorityVerifier` capability validates a versioned, signed approval record against the _current_ deletion-policy revision, covering: approver identity and eligible role; binding to this actor and this request; scope, classes, and targets; legal basis and reason; approval time and expiry; policy version; and revocation status. The actor performing execution may differ from the approver, but must be bound in the record.

Authority that is stale, expired, revoked, unverifiable, self-asserted, out of scope, or of unknown status **fails closed — no erasure.** The verifier adapter is itself a capability gate, and deterministic fakes must cover each of those failure modes.

**The inventory must be produced by an authority, not assembled ad hoc.** A mandatory, deterministic, revisioned `CustodyInventoryAuthority` enumerates the complete expected inventory across local, external, escrow, provider, key, and distributed-Git locations, reporting its source coverage, inventory revision, policy revision, and completeness evidence. PREPARE binds the exact inventory revision and digest.

A missing source, an omitted location class, a stale revision, a scan outage, a partial enumeration, or unknown completeness means **no erasure and no complete certificate.** Deterministic fakes must cover omission, outage, and staleness. This gate exists because an inventory that is merely _plausible_ is how a deletion comes to look complete while whole classes of copies were never enumerated.

Ordinary custody stays immutable. `RawArtifactStore` and `ReplayCapsuleStore` keep **no** delete, erase, purge, or unlink member; T-10-04 must not add one. Authorised erasure is a **separate, consented, human-authorised capability** — the sole named exception to the append-only rule — carrying its own expected-custody-revision check, legal-hold decision, immutable dependency-closure snapshot, the three-phase PREPARE → EXECUTE → FINALISE protocol with defined recovery (freeze happens in PREPARE, the certificate in FINALISE), and explicit idempotency and crash semantics. DESIGN §3's statement that the system "never makes immutable raw bytes deletable" should be read as scoped to ordinary custody stores, with this capability cross-referenced as the exception; DESIGN and this ADR are to be updated together so the two do not appear to contradict.

### Capability gates: acceptance does not unlock erasure

Accepting this ADR and ratifying D9 do **not** by themselves permit physical deletion. The workflow depends on producers that may not exist yet — the graph/validity gateway, the invalidation path, provider adapters, a certificate signer, the `DeletionAuthorityVerifier`, and the `CustodyInventoryAuthority` enumerator. Deletion is therefore **fail-closed on capability**: if any required producer is missing, unavailable, or of unknown status, no erasure proceeds.
