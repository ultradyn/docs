...
export interface TestingCoverageObligationEventWriterHooks {
  /** Deterministic crash seam after the ID-less claim is durable. */
  afterClaimPublished?: () => void | Promise<void>;
  /** Deterministic crash seam after a non-create operation reservation. */
  afterOperationReserved?: () => void | Promise<void>;
  /** Deterministic crash seam after immutable event publication, before intent commit. */
  afterEventPublished?: () => void | Promise<void>;
}

export type FileCoverageObligationEventWriterOptions = RepositoryLockOptions;

interface InternalFileCoverageObligationEventWriterOptions extends FileCoverageObligationEventWriterOptions {
...
async function readStream(
  ...
  for (const name of (await directory.list()).sort()) {
    ...
    if (
      ...
      throw new Error(
        `Coverage-obligation event belongs to stream ${event.obligationId}, not directory stream ${obligationId}.`,
      );
    }
...
class FileCoverageObligationEventWriter implements CoverageObligationEventWriter {
  ...
  readonly #options: InternalFileCoverageObligationEventWriterOptions;

  constructor(
    root: string,
    options: InternalFileCoverageObligationEventWriterOptions,
  ) {
  ...
  reserveCreate(
    ...
    return this.#locked(async (directories) => {
      ...
      await this.#options.hooks?.afterClaimPublished?.();
      const obligationId = ObligationIdSchema.parse(
        command.allocateObligationId(),
      );
      const reservation: AllocatedOperationRecord = {
  ...
  append(
    ...
    return this.#locked(async (directories) => {
      ...
      if (operation) {
        ...
        if (operation.commandDigest !== command.commandDigest) {
          return { status: "idempotency_conflict" };
        }
      } else {
        ...
        await this.#options.hooks?.afterOperationReserved?.();
      }
...
export function createFileCoverageObligationEventWriterImplementation(
  root: string,
...
/** @internal Exported only through repository/testing.ts, never the production barrel. */
export function createTestingFileCoverageObligationEventWriter(
  root: string,
  hooks: TestingCoverageObligationEventWriterHooks,
  options: FileCoverageObligationEventWriterOptions = {},
): CoverageObligationEventWriter {
  return new FileCoverageObligationEventWriter(root, { ...options, hooks });
}
