# Docent — Goals & Constraints

*Working title: **Docent** (a docent is a guide who answers questions). Rename freely.*
...
**P1 — Knowledge lives in heads and dies in chat logs.** Expert answers given in Slack/voice/hallways are high-value and unrecorded. Docent makes the low-friction medium (talking) produce the high-value artifact (consistent documentation) as a side effect.

**P2 — Documentation effort is misallocated.** Docs are traditionally written speculatively, depth-first, by guessing what readers need. Docent inverts this: unanswerable questions *are* the demand signal. Effort flows to proven gaps. Depth is added only where a real question requires it.

**P3 — Answering well is expensive; answering roughly is cheap.** Structuring, checking consistency, and cross-linking are exactly what experts skip. Docent lets the expert deliver an unstructured brain dump and pushes the expensive part (structure, criticism, integration) onto agents, with the expert only reviewing.

**P4 — "Answered" is usually undefined.** Docent makes it decisive: every question carries declared goals; an answer is done when it satisfies those goals with no contradiction against existing docs. Not perfect — good enough *for the declared goals*.
