...
- Proposal/approval mutations use expected revision and idempotency. Approval runs a fresh audit, unitizes old and corrected representations separately, and atomically appends the head transition, fresh audit, replacement units, decision record, and invalidation outbox request. The canonical `SourceFile` remains immutable; corrected locator bounds are qualified through a non-persisted view bound to the immutable `CorrectionArtifact` size/hash. This append is the commit point; recovery reads and acknowledges undelivered requests through an explicit repository-backed outbox port, returns exact request IDs, and cannot transition repair state.
